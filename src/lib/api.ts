...
export async function uploadPhoto(itemId: string, side: "front" | "back", file: File, processed = false) {
  ...
  let contentType = file.type || (/\.heic$/i.test(file.name) ? "image/heic" : /\.heif$/i.test(file.name) ? "image/heif" : "");
  if (!processed && !["image/heic", "image/heif"].includes(contentType)) {
    try {
      uploadFile = await compressImage(file, { maxDimension: 1800, quality: 0.86, mimeType: "image/webp" });
      contentType = "image/webp";
    } catch {
      // Keep the original browser file as a fallback for browsers without image decoding support.
    }
  }
  ...
  if (uploadFile.size > 20971520) throw new Error("Each photo must be 20 MB or smaller after compression.");
  ...
  form.append("", uploadFile, file.name.replace(/\.[^.]+$/, ".webp"));
