...
function AdminContents() {
  ...
  return <div className="wc-content wc-admin">
    ...
    {tab === "Setup" && <div className="wc-admin-columns"><div className="wc-form wc-surface"><h2>Daily Deals</h2><p className="wc-muted">Runs each morning, around 7–8am Arizona time. At most one successful scan per day. Research uses the OpenAI API.</p><button className="wc-button" disabled={busy} onClick={() => void run(() => requestJson("/api/deals", {}), "Daily research is ready.")}>{busy ? "Working…" : "Run today’s scan"}</button><button className="wc-button" disabled={busy} onClick={() => void run(() => requestJson("/api/deals", { demo: true }), "Demo deal preview loaded.")}>{busy ? "Working…" : "Load demo scan"}</button>{data.scans[0] && <p>Last scan: {data.scans[0].status}. {data.scans[0].isDemo ? "Demo preview. " : ""}{data.scans[0].error}</p>}<hr/><h2>Test catalog</h2><p className="wc-muted">Loads or refreshes the 25 published TEST-CLOTHES records and their private catalog images. Real items are not changed.</p><button className="wc-button" disabled={busy} onClick={() => void run(() => requestJson("/api/admin/test-catalog", {}), "25 test clothes are published.")}>{busy ? "Working…" : "Load 25 test clothes"}</button><button className="wc-text-link" onClick={() => void run(async () => { await requestJson("/api/session", { action: "logout" }); window.location.assign("/admin"); }, "")}>Sign out of admin</button></div><form className="wc-form wc-surface" onSubmit={saveSettings}><h2>Research & guide</h2><label>Stores<textarea name="stores" rows={4} defaultValue={data.settings.stores} maxLength={1500}/></label><label>Nearby area<input name="area" defaultValue={data.settings.area} maxLength={250}/></label><label>Manual PDF link<input name="manualUrl" type="url" placeholder="https://…" defaultValue={data.settings.manualUrl} maxLength={2000}/></label><button className="wc-button wc-button--accent" disabled={busy}>Save settings</button></form></div>}
    {selectedItem && <Drawer title={selectedItem.name} close={() => { if (!busy) setSelected(null); }}>
      <div className="wc-item-photos"><ItemPhoto item={selectedItem}/>{selectedItem.backUrl && <ItemPhoto item={selectedItem} side="back"/>}</div>
      <div className="wc-downloads">{(["front","back"] as const).map(side => { const url = side === "front" ? selectedItem.frontUrl : selectedItem.backUrl; return url ? <a key={side} className="wc-text-link" href={url + "&download=" + encodeURIComponent(selectedItem.id + "-" + side)} download>Save current {side} image</a> : <span key={side}>{side} image not uploaded yet</span>; })}</div>
      <div className="wc-form-grid wc-form">{(["front","back"] as const).map(side => <label key={side}>Replace {side} image<input type="file" accept="image/png,image/webp" disabled={busy} onChange={event => { const file = event.target.files?.[0]; event.target.value = ""; if (file) void run(() => uploadPhoto(selectedItem.id, side, file, true), "Item image replaced."); }}/></label>)}</div>
      ...
      <button className="wc-text-link wc-text-link--danger" disabled={busy} onClick={() => { if (confirm(`Delete ${selectedItem.name}? This cannot be undone.`)) void run(async () => { await mutate({ action: "removeItem", id: selectedItem.id }); setSelected(null); }, "Item deleted."); }}>Delete item</button>
    </Drawer>}
  </div>;
}
