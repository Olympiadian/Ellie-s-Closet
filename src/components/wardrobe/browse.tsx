...
export function ItemDetails({ item, close }: { item: WardrobeItem; close: () => void }) {
  ...
  return <Drawer title={current.name} close={close}>
    ...
    <dl className="closet-item-drawer__details">
      {Object.entries({ Topic: current.category, Tags: current.tags.join(" · "), Color: current.color, Details: [current.details, current.size && "Size " + current.size, current.store, current.cost !== null && "$" + current.cost.toFixed(2)].filter(Boolean).join(" · ") }).map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value || "Not added yet"}</dd></div>)}
    </dl>
