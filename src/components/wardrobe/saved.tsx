...
export function SavedPage() {
  ...
  return <PageShell title="Saved"><DataGate><div className="wc-tabs wc-saved-tabs">{["All", "Outfits", "Collections", "Items"].map(value => <button className={tab === value ? "is-active" : ""} aria-pressed={tab === value} key={value} onClick={() => setTab(value)}>{value}</button>)}</div>
    {!builds.length && !items.length && <Empty>Save an item from its details, or put together your first outfit in Build Outfit.</Empty>}
    <div className="wc-content wc-saved-grid">{builds.map(build => <button key={build.id} className="wc-saved-card" onClick={() => { setBuild(build); setMessage(""); }}><div className="wc-collage">{build.itemIds.slice(0, 4).map(id => { const item = data?.items.find(i => i.id === id); return item ? <ItemPhoto key={id} item={item} thumbnail/> : null; })}</div><strong>{build.name}</strong><small>{build.kind} · {build.occasion || build.itemIds.length + " items"}</small></button>)}{items.map(item => <button className="wc-saved-card" key={item.id} onClick={() => setItem(item)}><ItemPhoto item={item} thumbnail/><strong>{item.name}</strong><small>Saved item</small></button>)}</div>
