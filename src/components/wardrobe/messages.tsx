...
import { MessagesIcon } from "@/components/mobile/mobile-icons";
...
export function MessagesButton({ mobile = false }: { mobile?: boolean }) {
  ...
  return (
    <>
      <button
        className={mobile ? "mobile-home__quick-button" : "wc-utility-button"}
      ...
      >
        {mobile ? (
          <>
            <span>Messages</span>
            <MessagesIcon className="mobile-home__quick-icon" />
          </>
        ) : (
            ...
            {data?.messages.length ? data.messages.map((message) => (
              <article className="wc-message" key={message.id}>
                <time dateTime={message.date}>
                  {new Date(`${message.date}T12:00:00`).toLocaleDateString("en-US", { month: "long", day: "numeric" })}
                </time>
                <h3>{message.title}</h3>
                <p>{message.body}</p>
              </article>
            )) : <Empty>Your notes will arrive here.</Empty>}
          </DataGate>
        </Drawer>
      )}
    </>
