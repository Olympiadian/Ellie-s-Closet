...
export async function POST(request: Request) {
  try {
    ...
    if (!item) throw new AppError("Upload an image first.");
    const imagePath = item.frontProcessedPath ?? item.frontPath ?? item.backProcessedPath ?? item.backPath;
    if (!imagePath) throw new AppError("Upload an image first.");
